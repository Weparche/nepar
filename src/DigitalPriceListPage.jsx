...
const guideContent = {
  hr: {
    ...
    checks: ["Javno dostupni signali", "Bez pravne procjene", "Server-side provjera"],
  ...
  en: {
    ...
    checks: ["Public technical signals", "No legal assessment", "Server-side check"],
...
export default function DigitalPriceListPage() {
  ...
  return <main id="top" className="site-main font-sans text-slate-800">
    ...
    <section ref={checkerRef} className="content-section scroll-mt-20 px-4 pt-28 sm:pt-36">
      <div className="section-shell grid items-stretch gap-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(360px,.9fr)]">
        <div className="py-3 lg:py-8">
          ...
          <div className="mt-8 rounded-2xl bg-slate-950 p-5 text-white sm:p-7">
            <div className="flex items-start gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-cyan-300 text-blue-950"><Search size={20} aria-hidden="true" /></span><div><h2 className="text-2xl font-semibold tracking-[-0.025em]">{copy.checkerTitle}</h2><p className="mt-1 max-w-xl text-sm leading-6 text-slate-300">{copy.checkerLead}</p></div></div>
            <form onSubmit={check} noValidate className="mt-5 flex flex-col gap-3 sm:flex-row"><label className="sr-only" htmlFor="digital-price-list-url">{copy.checkerTitle}</label><input id="digital-price-list-url" className="min-h-14 w-full rounded-xl border border-white/20 bg-white px-4 text-base font-medium text-slate-950 outline-none transition placeholder:text-slate-500 focus:border-cyan-300 focus:ring-2 focus:ring-cyan-300" required type="url" value={url} onChange={(event) => setUrl(event.target.value)} placeholder={copy.checkerPlaceholder} inputMode="url" /><button className="button min-h-14 shrink-0 bg-cyan-300 text-blue-950 hover:bg-cyan-200" disabled={checking} type="submit">{checking ? copy.checkerLoading : copy.checkerButton}<ArrowRight size={18} aria-hidden="true" /></button></form>
            <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-xs font-medium text-slate-300">{guide.checks.map((item) => <span key={item} className="inline-flex items-center gap-1.5"><Check size={15} className="text-cyan-300" aria-hidden="true" />{item}</span>)}</div>
            {result && <div aria-live="polite" className={`mt-5 rounded-xl border p-5 ${statusColor}`}><div className="flex gap-3"><div className="mt-0.5">{result.status === "green" ? <CheckCircle2 className="text-emerald-700" aria-hidden="true" /> : <CircleAlert className={result.status === "red" ? "text-rose-700" : "text-amber-700"} aria-hidden="true" />}</div><div><h3 className="text-lg font-bold text-slate-950">{resultTitle}</h3><p className="mt-1 leading-6 text-slate-700">{resultBody}</p><div className="mt-4 grid gap-1.5 text-sm text-slate-700">{result.details?.pricePageUrl && <span>{copy.found}: <strong>{displayUrl(result.details.pricePageUrl)}</strong></span>}{result.details?.csvUrl ? <span>Pronađen CSV: <strong>{displayUrl(result.details.csvUrl)}</strong></span> : <span>{copy.csvMissing}</span>}{result.details?.xmlUrl ? <span>Pronađen XML: <strong>{displayUrl(result.details.xmlUrl)}</strong></span> : <span>{copy.xmlMissing}</span>}</div>{["yellow", "red"].includes(result.status) && !result.technical && <button className="button button-primary mt-5" onClick={() => goTo(formRef, true)}>{copy.implementation}<ArrowRight size={17} aria-hidden="true" /></button>}<p className="mt-4 text-xs leading-5 text-slate-600">{copy.technicalNotice}</p></div></div></div>}
          </div>
          <div className="mt-6 flex flex-col gap-3 sm:flex-row"><button className="button button-secondary" onClick={() => goTo(formRef, true)}>{copy.implementation}<ArrowRight size={18} aria-hidden="true" /></button><p className="self-center text-sm font-semibold text-slate-500">{copy.platformsLine}</p></div>
        </div>
        <aside className="relative min-h-[420px] overflow-hidden rounded-2xl bg-slate-100 lg:min-h-full">
          <img src="/brand/digital-price-list-proof-v1.png" alt={lang === "hr" ? "Ilustracija strukturiranog digitalnog cjenika" : "Illustration of a structured digital price list"} className="absolute inset-0 size-full object-cover" />
          <div className="absolute inset-x-0 bottom-0 bg-slate-950 p-6 text-white sm:p-7"><p className="text-sm font-semibold text-cyan-300">{copy.sourceTitle}</p><p className="mt-3 text-sm leading-6 text-slate-300">{copy.sourceMeta}</p><a href={sourceUrl} target="_blank" rel="noreferrer" className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-white underline decoration-cyan-300 underline-offset-4 hover:text-cyan-200">Narodne novine <ArrowRight size={16} aria-hidden="true" /></a></div>
        </aside>
      </div>
    </section>
